...
import utils.LM_utils.log_init as log_init
...
def increment_path(path, exp_name):
    # Increment path, i.e. runs/exp --> runs/exp{sep}0, runs/exp{sep}1 etc.
    if len(os.listdir(path)) == 0:
        return os.path.join(path, f"{exp_name}_0")
    else:
        num = max([int(p.split('_')[-1]) for p in os.listdir(path) if os.path.isdir(os.path.join(path, p))])  # similar paths
        return os.path.join(path, f"{exp_name}_{str(num + 1)}")
